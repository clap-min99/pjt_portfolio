...
에코 핀의 **상승엣지에서 타이머 시작, 하강엣지에서 거리 계산**합니다.
...
- PORTA가 초음파 Trig(PA0-2)와 FND2 자릿수(PA4-7)를 공유 → 비트 충돌 없음
- PORTF가 모터 방향(PF0-3)과 FND1 자릿수(PF4-7)를 공유 → 비트 충돌 없음
- 모터 배선이 반대로 꽂혀 `turn_left()`가 물리적으로 우회전 → 자율주행 로직 전체에 일관되게 적용되어 정상 동작
- L298N 구동 전원은 별도 12V 필요 (5V USB만으로는 모터 미동작)
