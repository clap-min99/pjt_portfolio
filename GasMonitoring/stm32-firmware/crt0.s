.syntax unified
	...
	/* ---------------------------------------------------- */
	/* 1. Cortex-M4 벡터 테이블 (Vector Table)              */
	/* ---------------------------------------------------- */
	.word	0x20020000                        @ 초기 스택 포인터 (MSP Top: 128KB SRAM 끝)
	.word	__start                           @ 리셋 핸들러 진입점
	...
	.word   TIM4_IRQHandler                   @ TIM4 (1ms SysTick)
	...
	.word   USART2_IRQHandler                 @ USART2 (Qt 통신)
	.word   Invalid_ISR                       @ Reserved
	...
	/* ---------------------------------------------------- */
	/* 2. 기본 예외 핸들러 (Default Handler 바인딩)         */
	/* ---------------------------------------------------- */
	.weak   Invalid_ISR
...
__start:
	...
	.equ GPIOA_LCKR,	0x4002001C @ ST-LINK 디버그 핀(PA13/PA14) 잠금 레지스터
	.equ RCC_AHB1ENR,  	0x40023830 @ GPIOA(Bit 0), GPIOC(Bit 2) 클럭 레지스터
	...
	@ ST-LINK 디버그 핀(PA13: SWDIO, PA14: SWCLK) 설정 고정(Lock)
	LDR		r0, =GPIOA_LCKR
	...
	/* ---------------------------------------------------- */
	/* 4. C 런타임 환경 초기화 (Data 복사 및 BSS 초기화)    */
	/* ---------------------------------------------------- */
	.extern __RO_LIMIT__
	.extern __RW_BASE__
	.extern __ZI_BASE__
